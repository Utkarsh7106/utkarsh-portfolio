// ── STATUS.JSON DYNAMIC LOADER ─────────────────────────────────────────────
// Reads status.json and populates the "Working On" and "Learning" sections.
// To update portfolio status: edit status.json and push to GitHub.
// Netlify auto-deploys within 60 seconds. No HTML edits needed.
fetch('status.json')
  .then(res => res.json())
  .then(data => {
    const workingList = document.getElementById('working-on-list');
    if (workingList && data.working_on) {
      workingList.innerHTML = data.working_on.map(p =>
        `<li><span class="currently-project">${p.name}</span><span class="currently-desc">${p.desc}</span></li>`
      ).join('');
    }
    const learningPills = document.getElementById('learning-pills');
    if (learningPills && data.learning) {
      learningPills.innerHTML = data.learning.map(skill =>
        `<span class="cpill">${skill}</span>`
      ).join('');
    }
  })
  .catch(() => {
    // Fail silently — sections remain empty if fetch fails (no UI break)
  });
// ───────────────────────────────────────────────────────────────────────────
...
const PROGRESS_DATA = {
  ...
  powerbi:{ pct: 50,  label: 'In Progress' },
